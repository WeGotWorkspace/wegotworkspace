...
export class EditorServer {
  ...
  /** Same-origin WebDAV pathname for the open document (Sabre files/…), when applicable. */
  ...
  /**
   * Open a document from same-origin WebDAV and keep {@code webdavResourcePath} so saves PUT back to that resource.
   */
  async openFromWebDav(
    ...
    const ft = opts.fileType || getFileExt(name) || "docx";
      ...
      const download = async () => {
        ...
        if (cfg) {
          const putPathname =
            ...
            filesResourcePathname(
              cfg.base_uri,
              `users/${cfg.username.replace(/[/\\]+/g, "_")}/${filenameForWebDavSave(cmd.title, this.title, this.fileType)}`,
            );
          try {
