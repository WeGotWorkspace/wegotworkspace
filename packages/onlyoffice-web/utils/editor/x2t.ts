...
export class X2tConverter {
  ...
  private sendMessage<T>(type: string, payload?: unknown): Promise<T> {
    return new Promise((resolve, reject) => {
      ...
      if (type === "convert" && (payload as { data?: unknown })?.data instanceof ArrayBuffer) {
        this.worker.postMessage({ id, type, payload }, [(payload as { data: ArrayBuffer }).data]);
      } else {
  ...
  private handleWorkerMessage = (event: MessageEvent<WorkerResponse>) => {
    ...
    if (type === "__sabre_x2t_error") {
      ...
      const err = new Error(d.error || "x2t init failed");
