...
- **Conflicts:** create one `createSyncConflictChannel<string>()` instance; report from flush, set the listener from your screen, and resolve in a modal. Contacts: field-level merge via [`contacts-conflict-dialog.tsx`](../src/contacts-core/src/contacts-conflict-dialog.tsx). Notes: binary Keep mine / Use server via [`notes-conflict-resolution.ts`](../src/lib/offline/notes-conflict-resolution.ts) and [`notes-conflict-dialog.tsx`](../src/notes-core/src/notes-conflict-dialog.tsx). Docs (editable text): concurrent edits merge via Yjs CRDT; deferred REST save retries on reconnect — no conflict dialog yet for save failures, rename, or delete.
- **Pending badge:** poll your "pending" selector and render a dot. See [`use-contacts-pending-sync.ts`](../src/contacts-core/src/use-contacts-pending-sync.ts), [`use-notes-pending-sync.ts`](../src/notes-core/src/use-notes-pending-sync.ts), [`use-docs-collab-pending-sync.ts`](../src/text-editor-core/docs-collab/use-docs-collab-pending-sync.ts).
- **Retry callout:** count failed (non-conflict) outbox rows with `isRetryableOutboxRow` and offer a Retry that flushes. See [`use-contacts-failed-sync.ts`](../src/contacts-core/src/use-contacts-failed-sync.ts), [`use-notes-failed-sync.ts`](../src/notes-core/src/use-notes-failed-sync.ts). Docs collab exposes [`use-docs-collab-failed-sync.ts`](../src/text-editor-core/docs-collab/use-docs-collab-failed-sync.ts) for the same signal (retry UI not wired yet).
