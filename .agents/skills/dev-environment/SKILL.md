...
| Login **HTTP 500** on `auth/token` | No `wgw-config.php` / `db.sqlite` (API falls back to empty in-memory DB) | `php packages/api/artisan wgw:dev-install` (also run by `pnpm dev:bootstrap` / `pnpm preview:bootstrap`) |
